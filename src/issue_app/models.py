...
class IssueRepository(object):
    ...
    def find_by_target_user(self, filter):
        ...
        if filter['projects'] is None:
            return Issue.objects.filter(models.Q(target_user=filter['user']) & models.Q(status=0))
        else:
            return Issue.objects.filter(models.Q(target_user=filter['user'])
                                    & models.Q(project__id=filter['projects'].id)
                                    & models.Q(name__icontains=filter['keyword'])
                                    & models.Q(status=filter['status'])
                                        )
